...
public class PostgresRegister implements Register {
    ...
    public void mintItems(Iterable<Item> items) {
        handle.useTransaction(TransactionIsolationLevel.SERIALIZABLE, (handle, status) -> {
            itemStore.putItems(items);
            ...
            entryLog.appendEntries(Lists.transform(records, r -> r.entry));
            recordIndex.updateRecordIndex(registerName, records);
        });
    }
    ...
    public Optional<Item> getItemBySha256(String sha256hex) {
        return dbi.withHandle(h -> itemStore.getItemBySha256(sha256hex));
    }
    ...
    public Collection<Item> getAllItems() {
        return dbi.withHandle(h -> itemStore.getAllItems());
    }
